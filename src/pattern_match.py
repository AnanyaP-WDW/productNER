...
# the idea is to make a decent matcher to develop training data
# and then use it to train a NER ML model (hashed CNN, attention based etc.)

# 1) make patterns and test them
# 2) pre-process incoming data --> reomove '(,), /,\,[,]' etc.
# 3) make training data
# 4) train hashed CNN model for NER


def pattern_fn():
    # matches patterns
    measure_pattern = [{'LIKE_NUM': True},
...
#doc = nlp("cantata (smash)hazelnut incense coffee beans 600 oz mocha blend")

# for the measurement
# for match_id, start_id, end_id in matcher(doc):
#     print(doc[start_id: end_id])
# # for the product name 
#     print(str(doc[:start_id])+ str(doc[end_id:]))
# # for the quantity

def parse_train_data_2(doc):
    ...
    #product_name_detections = [(doc[:start].start_char, doc[:start].end_char, 'PRODUCT_NAME') for idx, start, end in matcher(doc)]
...
def parse_train_data(doc):
    ...
    for idx, start, end in matcher(doc):
        match_string = str(doc[start:end])
        start_idx_overall, end_idx_overall = doc[start:end].start_char, doc[start:end].end_char
        re_match = re.match('(\d+)', match_string)
        start_num, end_num = re_match.span()
        gap = end_num - start_num
        # print(start_num, end_num)
        # print(match_string[start_num:end_num])
        span_length = start_idx_overall+gap
        # print(doc.text[start_idx_overall:start_idx_overall +gap])
        # print(doc.text[start_idx_overall+gap:end_idx_overall])


        value_of_measure = [(start_idx_overall, span_length,'value_of_measure')]
        unit_of_measure = [(span_length, end_idx_overall,'unit_of_measure')]
        

        measure_detections.extend(value_of_measure)
        measure_detections.extend(unit_of_measure)

    measure_detections.extend(product_name_detections)

    return (doc.text, {'entities': measure_detections})
#print(parse_train_data(nlp("el salvador fancy 1kg(SROC120CI0500A02) gobgebunswae/gajeongyongeseupeureso/mocha pot(M)")))
...
if __name__ == '__main__':
    ...
    [W6E3D3F]maxim mocha coffee mix 1kg maxim mocha coffee mix mokakeopimigseu 1kg maxim mocha co
    ...
    U C C gold special roasted beans rich blend 3 six 0 g
    bulk buying 3 set starbucks regular coffee light note blend 1 six 0 g nescafe
    free shipping carita 101 coffee filter 1~ for 2 people coffee beans 800g premium blend{jinha mountai
    atsushi ogawa specialty coffee powder 0 0 9 one 7 0 g[(mail service post posting)] [(free shipping n
    (lion coffee) tosutetsudo coconut one 9 eight gx5 pack (powder)
    mocha・irugachiehu(raw beans)[200g]
    avance AVANCE appraiser recommendation special blend (7.5gx18P)135g
    nescafe Cappuccino vanilla instant coffee box 310 g
    ...
    grande fire ship White Coffee 10 x 20g
    ...
    Carrefour mocha instant coffee mix box 18 g x 10
    ...
    Koffiehuis  Medium Roast Coffee  (1  x 750g)
    ...
    SAN FRANCISCO BAY queue league 2.0 coffee fad collection 80ct
    Folgers pauls coffee beulraegsilkeu queue league capsule 96 piece
    ...
    coffee Blue rich 200 capsules original for lavazza machines Blue - CAFFE' DIEM
    pop coffee 600 capsules compatible pods Mitaca Mps uttermost strong creamy and-your + CAFFE' DIEM
    ...
    Tchibo Cafissimo Caffe Crema Fine aroma coffee roasted ground in capsules 70 g (10 x 7 g)
    ...
    nlp.to_disk(r"/home/ananya/Desktop/python/Data_works/OpenCV_OCR/src/NER_experiment/custom_trained_models/with_unit_measure")

...
# at present we are using the inbuilt spacy training regime and model
...
def parse_train_data(doc):
    ...
    # regular expression for matching quantities
    quantity_match = re.search('(\d*\.?\d+)\s?(pack|piece|bottle|bag|capsule|can|t)', full_input_string)
    if quantity_match:

        digit_len = len(quantity_match.groups()[0])
        digit_span_start = int(quantity_match.span()[0])
        #digit_span = digit_span_start:digit_span_start + digit_len

        item_unit_span_end = int(quantity_match.span()[1])
        #item_unit_span = digit_span_start + digit_len: item_unit_span_end

        quantity_integer = [(digit_span_start, digit_span_start + digit_len,'quantity_integer')]
        item_unit = [(digit_span_start + digit_len, item_unit_span_end,'item_unit')]

        measure_detections.extend(quantity_integer)
        measure_detections.extend(item_unit)

    else:
        quantity_match = re.search('(x) *(\d+)', full_input_string)
        if quantity_match:
            digit_span_start = int(quantity_match.span()[0])
            #digit_span = digit_span_start:digit_span_start + 1

            item_unit_span_end = int(quantity_match.span()[1])
            #item_unit_span = digit_span_start + digit_len: item_unit_span_end

            #quantity_integer = [(digit_span_start, digit_span_start + 1,'quantity_integer')]
            quantity_integer = [(digit_span_start + 1, item_unit_span_end,'quantity_integer')]
...
if __name__ == '__main__':
    ...
    file1 = open(r"/home/ananya/Desktop/python/Data_works/productNER/src/train_data_2.txt", "w")
    ...
    nlp.to_disk(r"/home/ananya/Desktop/python/Data_works/OpenCV_OCR/src/NER_experiment/custom_trained_models/with_quantity_measure")
